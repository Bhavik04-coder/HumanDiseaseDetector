...
const BASE_PATH = '/knowledge-scroll'

interface KnowledgeScrollProps {
  onScrollComplete?: () => void;
}

export default function KnowledgeScroll({ onScrollComplete }: KnowledgeScrollProps) {
  ...
  return (
    <div ref={containerRef} className="relative h-[600vh]">
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        {/* Overlay Text */}
        <KnowledgeOverlayText scrollProgress={scrollYProgress} />
      </div>
    </div>
  )
}

function KnowledgeOverlayText({ scrollProgress }: { scrollProgress: any }) {
  const textSegments = [
    {
      range: [0.15, 0.25],
